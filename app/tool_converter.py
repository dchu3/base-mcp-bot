...
def convert_json_schema_to_gemini_schema(
    ...
    # Add description and enum if present
...
def mcp_tool_to_gemini_function(
    ...
    Args:
        client_name: Name of the MCP client (e.g., 'dexpaprika', 'honeypot')
        tool: MCP tool definition with 'name', 'description', 'inputSchema'

    Returns:
        Gemini FunctionDeclaration or None if conversion fails
    """
    try:
        ...
        # Namespace the function name: client_method (e.g., dexpaprika_getNetworkPools)
...
def parse_function_call_name(name: str) -> tuple[str, str]:
    ...
    Args:
        name: Function name like 'dexpaprika_getNetworkPools'

    Returns:
        Tuple of (client_name, method_name)
    """
